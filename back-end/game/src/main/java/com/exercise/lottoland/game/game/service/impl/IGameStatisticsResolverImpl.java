...
import com.exercise.lottoland.game.game.model.GameHelper.Result;
...
public class IGameStatisticsResolverImpl implements IGameStatisticsResolver{
    ...
    public GameStatisticsResponse resolve(Map<String, Player> players, AtomicLong gameStatistics) {
        ...
        response.setTotalWinsForPlayer1(players.get(Result.PLAYER_ONE.name()).getTotalWins());
        response.setTotalWinsForPlayer2(players.get(Result.PLAYER_TWO.name()).getTotalWins());
        response.setTotalDraws(players.get(Result.PLAYER_TWO.name()).getTotalDraws());
